...
export const rectDirection = (
  [c0, c1]:Rect
):Vector => vectorUnit(
  vectorSub(c1, c0)
)

export const rectRelativePosition = (
  rect:Rect,
  pos:Vector
):Coord2d => vectorSum(
  rect[0],
  vectorMult(pos, vectorSub(rect[1], rect[0]))
) as Coord2d
...
export const rectScale = (
  ...
  coord2dScale(c0, delta, origin),
  coord2dScale(c1, delta, origin)
])
...
export const rectContainsCoord = (
  ...
  return coords.every(([x, y]) => (
    intervalContainsValue(xInterval, x) &&
    intervalContainsValue(yInterval, y)
  ))
}
...
export const rectIntersectionOnY = rectIntersectsionOnAxis.bind(null, rectIntervalOnY)
export const rectIntersectsOnY = rectIntersectsOnAxis.bind(null, rectIntervalOnY)

export const rectIntersectionOnX = rectIntersectsionOnAxis.bind(null, rectIntervalOnX)
export const rectIntersectsOnX = rectIntersectsOnAxis.bind(null, rectIntervalOnX)

export const rectIntersectionRect = (rect, ...rects):(Rect | null) => {
...
export const rectIntersects = (
  rect:Rect,
  ...rects:Rect[]
):boolean => Boolean(rectIntersectionRect(rect, ...rects))

export const rectContainingCoords = (
  [first, ...coords]:Coord2d[]
):Rect => {
  const [[x0, x1], [y0, y1]] = coords.reduce(([xInterval, yInterval], [x, y]) => ([
...
export const rectIsS = (
  [, [refX1, refY1]]:Rect,
  [rect0]:Rect
):boolean => coord2dIsS([refX1, refY1 - 1], rect0)

export const rectIsN = (
  [ref0]:Rect,
  [, [rectX1, rectY1]]:Rect
):boolean => coord2dIsN(ref0, [rectX1, rectY1 - 1])

export const rectIsE = (
  [, [refX1, refY1]]:Rect,
  [rect0]:Rect
):boolean => coord2dIsE([refX1 - 1, refY1], rect0)

export const rectIsW = (
  [ref0]:Rect,
  [, [rectX1, rectY1]]:Rect
):boolean => coord2dIsW(ref0, [rectX1 - 1, rectY1])

export const rectDistanceX = (
  rect0:Rect,
  rect1:Rect
):number => (
  rectIsE(rect0, rect1)
