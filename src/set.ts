import { Interval, intervalContainsValue, intervalContains } from './interval'
...
export const setContains = (
  ...
  values.every((value) =>
    setIntervals.some((setInterval) => intervalContains(setInterval, value))
  )
 ...
 * ending at Infinity
 */
export const setComplement = (setIntervals: Set): Set =>
  setIntervals.length === 0
    ? [[-Infinity, Infinity]]
    : setIntervals.reduce(
        (complementIntervals, [currentStart, currentEnd], index) => {
          ...
          if (isFirst && isLast) {
            ...
            return [[-Infinity, currentStart - 1]]
          } else if (isLast) {
            ...
            return [...complementIntervals, [previousEnd + 1, currentStart - 1]]
          }
        },
        []
      )
 ...
 */
export const setAllIntervals = (set: Set): Set =>
  setComplement(set).reduce(
    (allIntervals, complementInterval, index) =>
      index === 0
        ? [complementInterval]
 ...
 */
export const setPosition = (set: Set, ...values: number[]): number[] => {
...
export const setUnion = (fromSet: Set, ...unionSets: Set[]): Set =>
  unionSets.reduce(
    (resultingSet, unionSetIntervals) =>
 ...
 */
export const set = (...intervals: Interval[]): Set => setUnion([], intervals)

export const setDifference = (fromSet: Set, ...differenceSets: Set[]): Set =>
  differenceSets.reduce(
    (resultingSet, differenceSetIntervals) =>
      differenceSetIntervals.reduce((resultingSet1, [start, end]) => {
        ...
        const dropStartReplacement: Interval | null =
          dropStartIndex !== null && start > resultingSet1[dropStartIndex][0]
            ? [resultingSet1[dropStartIndex][0], start - 1]
            : null

        const dropEndReplacement: Interval | null =
          dropEndIndex !== null && end < resultingSet1[dropEndIndex][1]
...
export const setIntersection = (
  ...
  intersectionSets.reduce(
    (resultingSet, intersectionSet) =>
